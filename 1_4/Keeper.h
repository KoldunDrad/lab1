...
void Keeper<T>::push(int index) {
	try {
		...
		ptr = tmp; //ptr указывает на новыймассив
			cin >> ptr[index]; //Вводим данные добавленногоэлемента, который находится под индексом index
			++size; //Увеличиваем размерконтейнера
...
void Keeper<T>::del(int id) {
	try {
		...
		T* tmp = new T[size - 1]; //Создаем новый массив на один элементменьше
			ptr[id] = ptr[size - 1]; //Копируем последний элемент на местоэлемента, который хотим удалить
			for (int i = 0; i < size - 1; ++i) { //Копируем элементы массива в новый массив, кромепоследнего
					tmp[i] = ptr[i];
			}
		delete[] ptr; //Удаляем старый массив
		ptr = tmp; //ptr указывает нановый массив
			--size; //Размерконтейнера уменьшился
...
void Keeper<T>::writefile(string s) {
	...
	try {
		...
		fout << size << endl; //Запись размераконтейнера
...
void Keeper<T>::readfile(string s) {
	delete[] ptr; //Очищаем массив, так какбудут записаны данные из файла
		ifstream fin(s, ios::in);
	fin >> size; //Считываем размерконтейнера в файле
		ptr = new T[size]; //Создаем массивразмером контейнера в файле
...
void Keeper<T>::record(int id) {
	try {
		...
		cin >> ptr[id]; //Вводим новые данныевыбранного элемента
	}
	catch (exception& ex) {
		cout << ex.what() << endl << endl; //Вывод ошибки на экран
	}
